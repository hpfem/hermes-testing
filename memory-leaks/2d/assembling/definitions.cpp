...
CustomWeakForm::CustomWeakForm(CustomRightHandSide1* g1, CustomRightHandSide2* g2, bool linear) : WeakForm<double>(2)
{
  if (linear)
  {
    this->add_matrix_form(new DefaultMatrixFormDiffusion<double>(0, 0, HERMES_ANY, nullptr, HERMES_SYM));

    this->fns_2d.push_back(new Hermes2DFunction<double>(-1.0));
    this->add_vector_form(new DefaultVectorFormVol<double>(0, HERMES_ANY, fns_2d.back()));

    this->add_matrix_form(new DefaultMatrixFormDiffusion<double>(1, 1, HERMES_ANY, nullptr, HERMES_SYM));

    this->fns_2d.push_back(new Hermes2DFunction<double>(-1.0));
    this->add_vector_form(new DefaultVectorFormVol<double>(1, HERMES_ANY, fns_2d.back()));
  }
  else
  {
    // Jacobian.
