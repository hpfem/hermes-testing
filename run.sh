...
if [ "$ans" = "y" ]; then
  ...
  make
  echo "Test built, running..."
  valgrind --leak-check=full --log-file=../03-transient-ValgrindLogfile ./03-transient
  echo "Valgrind output '03-transient-ValgrindLogfile' available in memory-leaks/"
